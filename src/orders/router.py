...
async def get_orders(
    ...
    try:
        query = (
            ...
            .where(order.c.date.between(start_date, end_date))
        )
...
async def get_pie_column(
    ...
    try:
        pie_query = (
            select(order.c.type, func.sum(order.c.amount))
            .where(order.c.user_id == user.id)
            .where(order.c.date.between(start_date, end_date))
            .group_by(order.c.type)
        )
        column_query = (
            select(order.c.date, func.sum(order.c.amount))
            .where(order.c.user_id == user.id)
            .where(order.c.date.between(start_date, end_date))
            .group_by(order.c.date)
        )
...
async def post_orders(
    ...
    try:
        ...
        return {"status": 200, "data": None, "details": None}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
