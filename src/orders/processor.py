...
codes = pd.read_csv("mcc.csv", sep=";").set_index("mcc").to_dict()["type"]
...
def parse_pdf(file, user: User) -> list[dict[Any, Any]]:
    ...
    if "Дата и\rвремя\rсовершения" in temp_df.columns:
        ...
        df["date"] = temp_df["Дата и\rвремя\rсовершения"].apply(
            lambda x: datetime.strptime(x, "%d.%m.%Y %H:%M:%S").date()
        )
        df["type"] = temp_df["МСС категория"].astype(int).map(codes) + " (БНБ)"
        df[["amount", "currency"]] = temp_df["Сумма в\rвалюте\rоперации"].str.split(
            " ", expand=True
        )
        df["amount"] = df["amount"].str.replace(",", ".").astype(float)
        df["user_id"] = user.id
    elif "Дата и время\rзапроса" in temp_df.columns:
        df["date"] = temp_df["Дата и время\rзапроса"].apply(
